...
<?php endif; ?>
<form method="post">
  <button type="submit" name="action" value="list_all">List All EOIs</button><br><br>
</form>

<form method="get">
  <label for="job_ref">Job Reference Filter:</label>
  <select name="search_job_ref" id="job_ref" onchange="this.form.submit()">
    <option value="">--Select--</option>
    <?php foreach ($job_refs as $ref): ?>
      <option value="<?php echo htmlspecialchars($ref, ENT_QUOTES, 'UTF-8'); ?>"
        <?php echo (isset($_GET['search_job_ref']) && $_GET['search_job_ref'] === $ref) ? 'selected' : ''; ?>>
...
<p>Find Applicant</p>
<form method="get">
  First Name: <input type="text" name="search_first_name" value="<?php echo isset($_GET['search_first_name']) ? htmlspecialchars($_GET['search_first_name'], ENT_QUOTES, 'UTF-8') : ''; ?>">
